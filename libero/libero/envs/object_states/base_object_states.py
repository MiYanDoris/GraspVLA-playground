...
class ObjectState(BaseObjectState):
    def __init__(self, env, object_name, is_fixture=False):
        ...
        self.init_pos = None
        self.recent_pos = deque(maxlen=5)
    ...
    def grasped(self):
        cur_pos = self.get_geom_state()["pos"]
        if self.init_pos is not None:
            return cur_pos[2] - self.init_pos[2] > 0.1
        else:
            if len(self.recent_pos) == 0:
                self.recent_pos.append(cur_pos)
                return False
            else:
                isStablized = True
                for pos in self.recent_pos:
                    if abs(pos[2] - cur_pos[2]) > 1e-2:
                        isStablized = False
                        break
                if isStablized:
                    self.init_pos = np.copy(cur_pos)
                    return False
                self.recent_pos.append(cur_pos)
        return False
