...
class Libero_Study_Tabletop_Manipulation(BDDLBaseDomain):
    ...
    def _check_success(self):
        ...
        elif goal_state[0] == 'or':
            result = False
            for state in goal_state[1:]:
                result = self._eval_predicate(state) or result
        else:
            raise ValueError(
                f"Goal state {goal_state} is not a valid logical expression."
            )
        return result
    ...
    def _setup_camera(self, mujoco_arena):
        mujoco_arena.set_camera(
            ...
            pos=[1.35554243 - 0.75, 0.        , 0.53883828 + 0.867 + 0.04],
            quat = [0.59636781, 0.3799282 , 0.3799282 , 0.59636781],
        ...
        mujoco_arena.set_camera(
            ...
            camera_attribs = {"fovy": "43"},
            pos = [0.5 - 0.75, 0.69282032, 0.5 + 0.867 + 0.04],
            quat = [0.0, 0.0 , -5.00000000e-01 , -8.66025404e-01]
        )
