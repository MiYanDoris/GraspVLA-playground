...
class IK_Solver:
    ...
    def __init__(self):
        robot_cfg = curobo.util_file.load_yaml('assets/franka_with_extended_finger/franka.yml')["robot_cfg"]
    ...
    def solve(self, abs_eef_action, current_joint):
        ee_position = torch.tensor(abs_eef_action[:3]).cuda().float()
        ee_quat = t3d.euler.euler2quat(abs_eef_action[3], abs_eef_action[4], abs_eef_action[5])
        ee_quaternion = torch.tensor(ee_quat).cuda().float()
...
class SingleArm(Manipulator):
    ...
    def control(self, action, policy_step=False):
        ...
        # Update the controller goal if this is a new policy step, use curobo to solve ik
        if policy_step:
            self.qpos_goal = self.IK_solver.solve(arm_action, np.array([self.sim.data.qpos[x] for x in self._ref_joint_pos_indexes]))
        ...
        self.sim.data.ctrl[:7] = self.qpos_goal
