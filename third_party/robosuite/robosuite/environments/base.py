...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def _initialize_sim(self, xml_string=None):
        ...
        # disable gravity for the environment
        self.sim.model.opt.gravity = (0, 0, 0)

        pseudo_gravity = np.zeros(6)
        pseudo_gravity[2] = -9.81 * 0.1

        # apply pseudo gravity to all objects, except for the robot
        for name in self.sim.model.body_names:
            if 'world' not in name and 'robot' not in name and 'mount' not in name and 'gripper' not in name:
                obj = self.sim.data.body(name)
